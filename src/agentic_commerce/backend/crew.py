...
class CommerceCrew:
    ...
    async def _catalog_scout(
        ...
        return await self.client.search_catalog(
            query=query, price_max=price_max_cents, limit=limit
        )

    async def _web_scout(self, query: str, max_price: float | None, limit: int) -> list[WebResult]:
        """Searches the open web for comparable listings."""
        phrase = f"{query} under ${max_price:.0f}" if max_price else query
        return await search_web(phrase, limit=limit)
    ...
    def run_discovery(
        ...
        if isinstance(catalog, Exception):
            ...
            report.catalog_products = catalog
            report.events.append(
                CrewEvent(
                    CATALOG_SCOUT,
                    "done",
                    f"{len(catalog)} catalog match(es)",
                    {"count": len(catalog)},
                )
            )
        ...
        if isinstance(web, Exception):
            ...
            report.web_results = web
            report.events.append(
                CrewEvent(WEB_SCOUT, "done", f"{len(web)} web listing(s)", {"count": len(web)})
            )
